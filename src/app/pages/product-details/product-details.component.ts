...
import { AbrstarctHttpService } from './../../services/abstarct-http/abrstarct-http.service';
...
import { HTTPReqArgs } from '../../services/abstarct-http/HTTPReqArgs';
...
export class ProductDetailsComponent implements OnInit {
  ...
  productsList: any;
  constructor(
    private abrstarctHttpService: AbrstarctHttpService,
  ...
  ngOnInit(): void {
    ...
    //   limit: 30,
    // };
    // this.listOfData = this.productsList.products;
    // this.sharedService.productsList = this.listOfData
  }

  fetchProdcuts() {
    const httpReq = new HTTPReqArgs();
    httpReq.url = 'https://dummyjson.com/products';
    httpReq.headers = new HttpHeaders();
    this.abrstarctHttpService.get$(httpReq).subscribe({
      next: (value: any) => {
        console.log('the value is', value);
        this.listOfData = value.products;
        this.sharedService.productsList = this.listOfData
      },
    });
  }
  ...
  handleCompare(obj: Products) {
    if (this.sharedService.selectedProductsArr.length < 4) {
      this.route.navigateByUrl('/compare-products');
